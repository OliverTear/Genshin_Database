...
for table in bsObj.findAll("table"):
    ...
    for row in Allrows:
        ...
        sqlitelist.append(f'INSERT INTO WeaponList(type, rarity, name, BASEATK, SubEffect, SubEffectValue, Support, Get) VALUES("{weapontype[0]}", "{row[2]}", {row[3]}, "{row[4]}", "{row[5]}", "{row[6]}", "{row[7]}", "{row[8]}", "{row[9]}", {row[10]}, {row[11]}, {row[12]}, "{row[13]}", "{row[14]}", {row[15]}, {row[16]})')
    weapontype.pop(0)