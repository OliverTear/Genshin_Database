    ...
    def add_character_view():
        def register_character(e):
            ...
            if charactername is None:
                page.open(error)
                error.content = ft.Text("キャラクターを選択してください")
                page.update()
                print("キャラクターを選択してください")
            elif levelvalue == "":
                page.open(error)
                error.content = ft.Text("レベルを入力してください")
                page.update()
                print("レベルを入力してください")
            else:
                cur.execute("select name from list where name LIKE '"+str(i)+"';")
                # キャラクターを登録する処理
                cur.execute('INSERT INTO Character(name, level) VALUES(?, ?)', (characterdropdown.value, level.value))
                conn.commit()
                page.open(error)
                error.content = ft.Text("キャラクターを登録しました")
                page.update()
                print("キャラクターを登録しました")
