    ...
    def add_weapon_view():
        ...
        def search(e):
            ...
            if kind == None and rarity == None and searchbar.value == "":
                ...
                cur.execute('SELECT * FROM WeaponList WHERE '+kind+' AND '+rarity+' AND name LIKE "%'+searchbar.value+'%"')
                ...
                if weaponresult == []:
                    ...
                    Dialog.content = ft.Text("該当する武器が"+str(len(cur.fetchall()))+"件あります")
                    rows = []
                    for data in weaponresult:
                        ...
                        cells = [ft.DataCell(ft.Text(data[1])),ft.DataCell(ft.Text(data[2])),ft.DataCell(ft.Text(data[3])),ft.DataCell(ft.Text(data[4]))]
                        rows.append(ft.DataRow(cells=cells))
                    header = [ft.DataColumn(ft.Text("キャラクター")), ft.DataColumn(ft.Text("レベル")), ft.DataColumn(ft.Text("武器")), ft.DataColumn(ft.Text("聖遺物"))]
