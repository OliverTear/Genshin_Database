    ...
    def list_weapon_view():
        ...
        def read_data(e):
            ...
            for data in characterdata:
                ...
                opcount += 1
                cells = [ft.DataCell(ft.Text(data[1])),ft.DataCell(ft.Text(data[2])),ft.DataCell(ft.Text(data[3])),ft.DataCell(ft.Text(data[4])),ft.DataCell(editbutton), ]
                rows.append(ft.DataRow(cells=cells))
            data_table.rows = rows
            page.update()
        header = [ft.DataColumn(ft.Text("キャラクター")), ft.DataColumn(ft.Text("OptionName"))]
        rows = []
