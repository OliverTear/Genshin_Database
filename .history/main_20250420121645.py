...
def read_characterlist():
    ...
    for row in Allrows:
        if row[3] == "☆5":
            ...
            row[3] = "4"
        sqlitelist.append(f'INSERT INTO CharacterList(kana, name, rarity, element, weapon, sex , birthday, country, region, HP, ATK, DEF, OriginalValue, CollectionValue, ElementalEnergy) VALUES("{row[1]}", "{row[2]}", {row[3]}, "{row[4]}", "{row[5]}", "{row[6]}", "{row[7]}", "{row[8]}", "{row[9]}", {row[10]}, {row[11]}, {row[12]}, "{row[13]}", "{row[14]}", {row[15]})')
