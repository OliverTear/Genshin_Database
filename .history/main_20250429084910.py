        ...
        def search(e):
            ...
            if kind == None and rarity == None and searchbar.value == "":
                ...
                if kind == None:
                    ...
                    if rarity == None and searchbar.value ==None:
                        kind = "type LIKE '%"+kind+"%'"
                    else:
                        kind = "type LIKE '%"+kind+"%' AND "
                if rarity == None:
                    rarity = ""
                else:
                    if searchbar.value == None:
                        rarity = "rarity LIKE '%"+rarity+"%'"
                    else:
                        rarity = "rarity LIKE '%"+rarity+"%' AND "
                if searchbar.value == None:
                    searchbar.value = ""
                else:
                    searchbar.value = "name LIKE '%"+searchbar.value+"%'"
                cur.execute('SELECT * FROM WeaponList WHERE '+kind+rarity+searchbar.value)
                weaponresult = cur.fetchall()
                if weaponresult == []:
                    page.open(Dialog)
                    Dialog.title=ft.Text("エラー")
                    Dialog.content = ft.Text("該当する武器がありません")
                    page.update()
                elif len(weaponresult) == 1:
                    searchbar.value = data[0][2]
                else:
                    Dialog.title=ft.Text("検索結果")
                    ...
                    for data in weaponresult:
                        ...
                        cells = [ft.DataCell(ft.Text(data[1])),ft.DataCell(ft.Text(data[2])),ft.DataCell(ft.Text(data[3]))]
                        rows.append(ft.DataRow(cells=cells))
                    header = [ft.DataColumn(ft.Text("武器種")), ft.DataColumn(ft.Text("レアリティ")), ft.DataColumn(ft.Text("名前"))]
