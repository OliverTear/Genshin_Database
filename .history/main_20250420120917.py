...
def read_characterlist():
    ...
    for row in Allrows:
        if row[3] == "☆5":
            ...
            row[3] = "4"
        sqlitelist.append(f'INSERT INTO kana("{row[1]}") name("{row[2]}") rarity({row[3]}) element("{row[4]}") weapon("{row[5]}") sex("{row[6]}") birthday("{row[7]}") country("{row[8]}") region("{row[9]}") HP({row[10]}) ATK({row[11]}) DEF({row[12]}) OriginalValue("{row[13]}") CollectionValue("{row[14]}") ElementalEnergy({row[15]}) Version({row[16]})')
    return sqlitelist
