...
def main(page: ft.Page):
    ...
    def add_weapon_view():
        ...
        def search(e):
            global kind, rarity
            def decide_weapon(e):
                registertext.value = e.control.data[2]
                Dialog.open = False
                page.update()
            ...
            if kind == None and rarity == None and searchbar.value == "":
                ...
                if rarity == "全て":
                    rarity = None
                if kind == None and rarity == None and (searchbar.value == None or not searchbar.value):
                    cur.execute('SELECT * FROM WeaponList')
                else:
                    if kind == None:
                        kindsql = ""
                    ...
                    if rarity == None:
                        ...
                        if searchbar.value == None or not searchbar.value:
                            raritysql = "rarity LIKE "+rarity[1]
                        else:
                            raritysql = "rarity LIKE "+rarity[1]+" AND "
                    if searchbar.value == None or not searchbar.value:
                        searchbarvaluesql = ""
                    else:
                        if searchcondition.value == "を含む":
                            searchbarvaluesql = "name LIKE '%"+searchbar.value+"%'"
                        elif searchcondition.value == "を含まない":
                            searchbarvaluesql = "name NOT LIKE '%"+searchbar.value+"%'"
                        elif searchcondition.value == "で始まる":
                            searchbarvaluesql = "name LIKE '"+searchbar.value+"%'"
                        elif searchcondition.value == "で終わる":
                            searchbarvaluesql = "name LIKE '%"+searchbar.value+"'"
                        elif searchcondition.value == "完全一致":
                            searchbarvaluesql = "name LIKE '"+searchbar.value+"'"
                ...
                if weaponresult == []:
                    ...
                    Dialog.title=ft.Text("エラー")
                    Dialog.actions = [
                        ft.Text("該当する武器がありません"),
                        ft.TextButton("閉じる",on_click=close_dlg),]
                    page.open(Dialog)
                    page.update()
                elif len(weaponresult) == 1:
                    registertext.value = weaponresult[0][2]
                    page.update()
                else:
                    rows = []
                    for data in weaponresult:
                        ...
                        cells = [ft.DataCell(ft.Text(data[0])),ft.DataCell(ft.Text(data[1])),ft.DataCell(ft.Text(data[2])),ft.DataCell(ft.Text(data[3])),ft.DataCell(ft.Text(data[4])),ft.DataCell(ft.Text(data[5])), ft.DataCell(checkbutton)]
                        rows.append(ft.DataRow(cells=cells))
                    header = [ft.DataColumn(ft.Text("武器種")), ft.DataColumn(ft.Text("星")), ft.DataColumn(ft.Text("名前")), ft.DataColumn(ft.Text("基礎攻")),ft.DataColumn(ft.Text("サブ効果")),ft.DataColumn(ft.Text("サブ効果値")), ft.DataColumn(ft.Text("決定"))]
                    data_table = ft.DataTable(columns=header, rows=rows)
                    Dialog.actions = [
                        ft.TextButton("閉じる",on_click=close_dlg),]                    
                    Dialog.title = ft.Column([data_table])
                    page.open(Dialog)
        ...
        def register_weapon(e):
            ...
            print(weaponname, level, talent, None)
            if weaponname == "武器を選択してください":
                ...
                if type(cur.fetchone()) == types.NoneType:
                    # 武器を登録する処理
                    cur.execute('INSERT INTO Weapon(name) VALUES(?, ?, ?, ?)', (weaponname, level, talent, "None"))
                    conn.commit()
                    page.open(Dialog)
                    Dialog.title=ft.Text("登録完了")
                    Dialog.content = ft.Text("武器を登録しました")
                    page.update()
                else:
                    page.open(Dialog)
                    Dialog.title=ft.Text("エラー")
        ...
        searchcondition= ft.Dropdown(
            label="検索方法",
            options=[
                ft.dropdown.Option("を含む"),
                ft.dropdown.Option("を含まない"),
                ft.dropdown.Option("で始まる"),
                ft.dropdown.Option("で終わる"),
                ft.dropdown.Option("完全一致"),
            ],
            value="を含む",
            border_radius=10,
            border_color=ft.colors.AMBER,
